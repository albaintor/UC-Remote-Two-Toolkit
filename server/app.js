...
app.get('/server/api', (req, res, next) => {
    ...
    got.get(url, options).then(proxyres => {
        ...
        try {
            if (proxyres?.body)
                resBody = JSON.parse(proxyres.body);
        }
        catch (err) {
...
app.post('/api/remote/:address/wake', async (req, res, next) => {
    ...
    const remote = new Remote(remoteEntry.address, remoteEntry.port, remoteEntry.user, remoteEntry.token, remoteEntry.api_key, remoteEntry.mac_address);
    try {
...
app.get('/api/proxy', async (req, res) => {
    ...
    await streamPipeline(got.get(encodeURI(decodeURI(url)), options), res);
    res.end();
});
