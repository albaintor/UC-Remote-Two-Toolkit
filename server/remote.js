...
export class Remote {
    ...
    async loadResources(type, resources_directory) {
        ...
        for (let icon of icons) {
            ...
            await streamPipeline(got.stream(`${url}/${icon['id']}`, options), fs.createWriteStream(path.join(target_path, icon['id'])));
        }
    ...
    toJson() {
        ...
        if (this.api_key_name)
            data.api_key_name = this.api_key_name;
        if (this.valid_to)
    ...
    async wakeOnLan(broadcast = undefined) {
        if (!this.mac_address) {
    ...
    async getBackup(response) {
        ...
        await streamPipeline(downloadStream, response);
        response.end();
        /*downloadStream
